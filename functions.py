...
def validate_tables():
    if dsa == 'SQLite':
        ...
        db = pymysql.connect(host=mysql_connection['host'], user=mysql_connection['user'], password=mysql_connection['password'], database=mysql_connection['database'])
        cursor = db.cursor()
    else:
        raise Exception('Invalid database type (dsa). Please check your config.py file.')
